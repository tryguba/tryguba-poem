 ...
 * @return array
 */
function malcolmy_lite_share_buttons( $context = 'loop', $args = array(), $config = array() ) {
	...
	if ( ! malcolmy_lite_is_meta_visible( $meta, $context ) ) {
		return;
	}
	...
	$default_config = apply_filters( 'malcolmy_lite_default_config_share_buttons', array(
	...
	$share_item_html = apply_filters( 'malcolmy_lite_share_button_html',
	...
	foreach ( (array)$networks as $id => $network ) :
		...
		$share_buttons .= sprintf(
			...
			esc_html__( 'Share on ', 'malcolmy_lite' ) . $network['name'],
