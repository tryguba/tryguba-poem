 ...
 */
class Malcolmy_Subscribe_Follow_Widget extends Cherry_Abstract_Widget {
	...
	public function __construct() {
		...
		$this->widget_description = esc_html__( 'Display subscribe form and follow links.', 'malcolmy' );
		$this->widget_id = 'malcolmy_widget_subscribe_follow';
		$this->widget_name = esc_html__( 'Subscribe and Follow', 'malcolmy' );
		$this->settings = array(
			'enable_subscribe'        => array(
				...
				'options' => array(
					'enable_subscribe' => esc_html__( 'Enable Subscribe Box', 'malcolmy' ),
				),
			),
			'subscribe_title'         => array(
				'type'  => 'text',
				'value' => esc_html__( 'Subscribe', 'malcolmy' ),
				'label' => esc_html__( 'Subscribe Title', 'malcolmy' ),
			),
			'subscribe_message'       => array(
				'type'  => 'textarea',
				'label' => esc_html__( 'Subscribe text message', 'malcolmy' ),
			),
			'subscribe_input'         => array(
				'type'  => 'text',
				'value' => esc_html__( 'Enter your e-mail', 'malcolmy' ),
				'label' => esc_html__( 'Subscribe input placeholder', 'malcolmy' ),
			),
			'subscribe_submit'        => array(
				'type'  => 'text',
				'value' => esc_html__( 'Subscribe', 'malcolmy' ),
				'label' => esc_html__( 'Subscribe submit label', 'malcolmy' ),
			),
			'subscribe_success'       => array(
				'type'  => 'text',
				'value' => esc_html__( 'You successfully subscribed', 'malcolmy' ),
				'label' => esc_html__( 'Subscribe success', 'malcolmy' ),
			),
			'enable_follow'           => array(
				'type'    => 'checkbox',
				'value'   => array(
				...
				'options' => array(
					'enable_follow' => esc_html__( 'Enable Follow Box', 'malcolmy' ),
				),
			),
			'follow_title'            => array(
				'type'  => 'text',
				'value' => esc_html__( 'Follow', 'malcolmy' ),
				'label' => esc_html__( 'Follow Title', 'malcolmy' ),
			),
			'follow_message'          => array(
				'type'  => 'textarea',
				'label' => esc_html__( 'Follow text message', 'malcolmy' ),
			),
			'enable_background'       => array(
				...
				'options' => array(
					'enable_background' => array(
						'label' => esc_html__( 'Enable Custom Background', 'malcolmy' ),
			...
			'background_image'        => array(
				'type'               => 'media',
				'label'              => esc_html__( 'Background Image', 'malcolmy' ),
				'upload_button_text' => esc_html__( 'Choose Image', 'malcolmy' ),
				'multi_upload'       => false,
			...
			'invert_text_colorscheme' => array(
				...
				'master'  => 'background_image',
				'options' => array(
					'invert_text_colorscheme' => esc_html__( 'Use "Invert scheme" for text color', 'malcolmy' ),
				),
			),
			'background_position'     => array(
				'type'    => 'select',
				'label'   => esc_html__( 'Background Position', 'malcolmy' ),
				'value'   => 'center',
				'options' => array(
					'top-left'      => esc_html__( 'Top Left', 'malcolmy' ),
					'top-center'    => esc_html__( 'Top Center', 'malcolmy' ),
					'top-right'     => esc_html__( 'Top Right', 'malcolmy' ),
					'center-left'   => esc_html__( 'Middle Left', 'malcolmy' ),
					'center'        => esc_html__( 'Middle Center', 'malcolmy' ),
					'center-right'  => esc_html__( 'Middle Right', 'malcolmy' ),
					'bottom-left'   => esc_html__( 'Bottom Left', 'malcolmy' ),
					'bottom-center' => esc_html__( 'Bottom Center', 'malcolmy' ),
					'bottom-right'  => esc_html__( 'Bottom Right', 'malcolmy' ),
				),
				'master'  => 'background_image',
			),
			'background_repeat'       => array(
				...
				'value'   => 'no-repeat',
				'options' => array(
					'repeat'    => esc_html__( 'Repeat', 'malcolmy' ),
					'repeat-x'  => esc_html__( 'Repeat X', 'malcolmy' ),
					'repeat-y'  => esc_html__( 'Repeat Y', 'malcolmy' ),
					'no-repeat' => esc_html__( 'No repeat', 'malcolmy' ),
				),
				'master'  => 'background_image',
			),
			'background_size'         => array(
				'type'    => 'select',
				'label'   => esc_html__( 'Background Size', 'malcolmy' ),
				'value'   => 'inherit',
				'options' => array(
					'cover'   => esc_html__( 'Cover', 'malcolmy' ),
					'contain' => esc_html__( 'Contain', 'malcolmy' ),
					'auto'    => esc_html__( 'Auto', 'malcolmy' ),
				),
				'master'  => 'background_image',
			),
			'background_color'        => array(
				'type'   => 'colorpicker',
				'label'  => esc_html__( 'Background Color', 'malcolmy' ),
				'master' => 'background_image',
			),
		);

		add_action( 'wp_ajax_malcolmy_subscribe', array( $this, 'process_subscribe' ) );
		add_action( 'wp_ajax_nopriv_malcolmy_subscribe', array( $this, 'process_subscribe' ) );

		$this->errors = array(
			'nonce'     => esc_html__( 'Security validation failed', 'malcolmy' ),
			'mail'      => esc_html__( 'Please, provide valid mail', 'malcolmy' ),
			'mailchimp' => esc_html__( 'Please, set up MailChimp API key and List ID', 'malcolmy' ),
			'internal'  => esc_html__( 'Internal error. Please, try again later', 'malcolmy' ),
		);
	...
	public function widget( $args, $instance ) {
		...
		if ( $subscribe_enabled && $subscribe_template && $api_key && $list_id ) {
			...
			esc_html_e( 'Please set up MailChimp API key and List ID', 'malcolmy' );
		}
	...
	public function get_social_nav() {
		return malcolmy_get_social_list( 'widget' );
	}
	...
	public function get_subscribe_submit( $class ) {
		...
		$subscribe_submit = malcolmy_render_icons( $subscribe_submit );
	...
	public function process_subscribe() {
		...
		if ( ! isset( $_POST['nonce'] ) || ! wp_verify_nonce( $_POST['nonce'], 'malcolmy_subscribe' ) ) {
...
add_action( 'widgets_init', 'malcolmy_register_subscribe_follow_widgets' );
function malcolmy_register_subscribe_follow_widgets() {
	register_widget( 'Malcolmy_Subscribe_Follow_Widget' );
}
